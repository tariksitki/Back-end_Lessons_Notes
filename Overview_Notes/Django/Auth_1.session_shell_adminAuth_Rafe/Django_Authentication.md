...
INSTALLED_APPS = [
    ...
    'django.contrib.auth',  # Core authentication framework and its default models. (bu modül django auth un temelidir.)

    'django.contrib.contenttypes',  # Django content type system (allows permissions to be associated with models). yetkilendirme ile ilgili bir application dir ve otomatik gelir
    ....

MIDDLEWARE = [
    # bunlar da bizim haberimiz olmadan arka planda calisirlar
    ...
...
- User,
- Permission,
- Group Models, ## bunlar django nun auth modelleridir
...
## bu fotolara bakalim!!! fotoda auth modellerine ait class lar var
...
## create_user bir func dir. bu func ile olusturulan user otomatik db ye kaydedilir.
## biz create_user(tarik, tariksitki0119@gmail.com, 12345 yazdigimizda otomatik algilanir ama best practice olarak,
# sadece isim de key kullanilmaz direkt isim yazilir ama digerlerinde önce key sonra value yazilir. cünkü ekstra field lar girecegimiz icin karisma olabilir.
# )
...
## Biz password yazmadigimizda, otomatik bir sifre atanmaz. sadece django ic islemlerini yapmak icin karakterler bütününden olusan gececi sifre atar. Ama biz admin panele gittigimizde yine password yok gösterecektir.
...
user1 = User.objects.create_user("ali", "ali@gmail.com")
burada user1.email dedigimizde email algilanir.

## Not: user1 = User.objects.all() calistiginda tüm kullanicilari user1 e esitler.

## default da lastname ile ilgili bir alan yok. bunun icin kendimiz key value ile olustururuz.
...
### password degisikligi (3, 4 secenek var)

```py
python manage.py changepassword <username>
## Bu kod; shell den degil normal terminal den password degistirmek icin kod
## bu kod yazildiktan sonra yeni sifre girmemiz istenir.
...
# bu kod, create_user gibi bir kod, sifre degisikligi icin özel.
user3.save()
```
...
## Su ana kadar programatik olarak user olusturmayi gördük. Ama bu isin esasi; user lari kendi kendilerine login ve register olmalaridir. Simdi bunu görecegiz.

We want to allow adding regular users to our app.
Projemize user ekleyebilmek icin, django bize sadece urls.py a asagidaki kodu eklememizi söylüyor. iki urls.py a da eklenebilir. biz projeninkine ekledik
...
```
## asagidaki path lerin her birini yazdigimizda bizi karsisindaki yere gönderecek

accounts/ login/ [name='login']
## mesela bu url i yazdigimizda, bizden registration altinda bir login template i yazmamizi isteyecek.

## burada bulunan bütün url ler icin django tarafindan bir template hazirlanmistir

accounts/ logout/ [name='logout']
## default daki logout sayfasi biraz sikintili. Cünkü burada bir home tusu var. logout olmus kullaniciyi da admin sayfasina yönlendiriyor.
...
### Login template
    ...
    Docs a göre login islemleri icin view yazmiyoruz. Herseyi bize django sagliyor. ve bize login icin otomatik olarak form gönderiyor.
...
## Bir user ile login oldugumuzda, django bize otomatik olarak o kisinin profil sayfasina gitmemmiz gerektigini söylüyor. ama bunu degistirebiliyoruz. bunun icinde asagidaki kodu settings.py da yaziyoruz.
# burada homepage e gitmesini istiyorsak iki sey yapabiliriz. birincisi / koymak ikincisi ise urls.py daki name leri kullanmak yani home yazmak.
# peki bunu neden settings de yapiyoruz. Cünkü login icin biz bir view hazirlamadik. Bunun icin bir environment variable hazirlanmis ve bu da settings e yazilmasi gerekir.
## ikinci yöntem olarak hoca birde account sayfasi olusturabilirsiniz dedi.
...
## login icin view yazmamistik ama register icin yazmamiz gerekir.
...
Password reset icin gercek olmayan bir email almak istiyoruz. Bunun icin settings.py a bir komut yazmamiz gerekir. Birkac secenegimiz vardir. secenekler asagida. sadece seceneklerin code larini settings.py a yazmamiz yeterli
...
Bu yöntemde, email terminalde görüntülenir. Diger yöntemde ise, projenin ana dizininde sent_emails adinda klasör olusur ve onun altina gelir
...
sadece bu decorator i kullanarak bir sayfaya girisi engelleyebiliriz.
homepage e herkes girsin ama special.html sayfasina herkesin gitmesini istemiyoruz. Bu durumda bu kullanilir. Öncelikle view.py da import ediyoruz daha sonra kullaniyoruz.
